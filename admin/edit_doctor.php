...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!empty($_POST['password'])) {
            ...
            $pass_stmt->bind_param('si', $password_hash, $doctor_id);
            $pass_stmt->execute();
        }
        ...
        // Delete existing degrees and add new ones
        $delete_degrees = $conn->prepare("DELETE FROM degrees WHERE doctor_id = ?");
        $delete_degrees->bind_param('i', $doctor_id);
        $delete_degrees->execute();
...
<head>
    ...
    <title>Edit Doctor - Medilinx Admin</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="medical_license_number" name="medical_license_number" value="<?php echo htmlspecialchars($doctor['medical_license_number'] ?? ''); ?>">
            </div>

            <div class="form-group">
                <label for="years_of_experience">Years of Experience</label>
                <input type="number" id="years_of_experience" name="years_of_experience" value="<?php echo (int)($doctor['years_of_experience'] ?? 0); ?>" min="0">
            </div>

            <div class="form-group">
                <label for="languages_spoken">Languages Spoken</label>
                <input type="text" id="languages_spoken" name="languages_spoken" value="<?php echo htmlspecialchars($doctor['languages_spoken'] ?? ''); ?>">
            </div>

            <div class="form-group">
                <label for="work_address">Work Address</label>
                <textarea id="work_address" name="work_address"><?php echo htmlspecialchars($doctor['work_address'] ?? ''); ?></textarea>
            </div>

            <div class="form-group">
                <label for="consultation_hours">Consultation Hours</label>
                <textarea id="consultation_hours" name="consultation_hours"><?php echo htmlspecialchars($doctor['consultation_hours'] ?? ''); ?></textarea>
            </div>

            <div class="form-group">
                <label for="professional_biography">Professional Biography</label>
                <textarea id="professional_biography" name="professional_biography"><?php echo htmlspecialchars($doctor['professional_biography'] ?? ''); ?></textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <?php if (!empty($doctor['profile_image'])): ?>
                    <div>
                        <img src="../<?php echo htmlspecialchars($doctor['profile_image']); ?>" alt="Current profile image" style="max-width: 150px; margin: 10px 0;">
                    </div>
                <?php endif; ?>
                ...
                <?php if (count($degrees) > 0): ?>
                    <?php foreach ($degrees as $index => $degree): ?>
                        <div class="degree-form">
                            <div class="form-group">
                                <label>Degree</label>
                                <input type="text" name="degree_name[]" value="<?php echo htmlspecialchars($degree['degree_name']); ?>">
                            </div>
                            <div class="form-group">
                                <label>Institution</label>
                                <input type="text" name="institution[]" value="<?php echo htmlspecialchars($degree['institution']); ?>">
                            </div>
                            <div class="form-group">
                                <label>Year</label>
                                <input type="number" name="passing_year[]" value="<?php echo (int)$degree['passing_year']; ?>" min="1900" max="<?php echo date('Y'); ?>">
                            </div>
            ...
            <div style="margin-top: 20px;">
                ...
                <a href="view_doctor.php?id=<?php echo $doctor_id; ?>" class="btn btn-secondary" style="text-decoration: none;">Cancel</a>
            </div>
        </form>
    </div>
